...
#### `batch_create_concepts` (Batch)

Efficiently creates multiple concepts in a single operation. Includes guardrails to prevent errors and validate before creation.

Parameters:

- `dryRun` (optional, default: false): If true, validates all items without creating them
- `items` (required): Array of concepts to create (max 50):
  - `term` (required): Main term/title
  - `description` (required): Markdown content
  - `curriculumNodeCode` (required): Parent node code
  - `slug` (optional): URL-friendly ID (auto-generated if omitted)
  - `termEn` (optional): English term
  - `descriptionEn` (optional): English description

**Guardrails:**

- Maximum 50 items per batch to prevent runaway operations
- All items are validated before any are created
- Items grouped by node for efficient ordinal calculation
- Detailed results returned for each item

```
User: "Create 5 concepts about sail types for LBS-2-1A"
AI: "I'll run a dry run first to validate, then create them..."
...
1. Use `dryRun: true` first to validate all items
2. Review the validation results
3. If all items pass validation, create with `dryRun: false`
