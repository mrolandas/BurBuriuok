...
- Manual smoke tests target backend APIs and seed regeneration integrity:
  ...
  - Visit `/admin/media` to confirm concept/type/source filters, cursor pagination, and search behave correctly; create both an upload and external media item, ensure the upload succeeds without manual token handling, fetch a signed URL, and remove the asset while observing success toasts and updated counts.
  - While editing an existing concept, use the “Papildoma medžiaga” panel to confirm the attachment list loads, create a new media entry via the drawer (concept locked), and delete an attachment—verifying the Concept Manager refreshes immediately and the global media workspace reflects the change after reload.
