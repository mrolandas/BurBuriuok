...
- Manual smoke tests target backend APIs and seed regeneration integrity:
  ...
  - Visit `/admin/media` to confirm concept/type/source filters, cursor pagination, and search behave correctly; create both an upload and external media item, ensure the upload succeeds without manual token handling, fetch a signed URL, preview the asset inside the drawer, exercise checkbox multi-select plus bulk delete (verifying concept attachments stay in sync), and remove the asset while observing success toasts and updated counts.
