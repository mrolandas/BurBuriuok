...
- **Integrations** – Supabase REST only; device-key header authenticates learner progress writes. Runtime Supabase URL/anon key are sourced from `window.__BURBURIUOK_CONFIG__` so Pages builds remain static. No auth or storage buckets yet; media lives as external links referenced from concept content.
