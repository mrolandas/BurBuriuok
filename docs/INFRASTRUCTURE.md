...
- **Hosting** – SvelteKit frontend lives under `frontend/` (local dev via Vite) and ships as a static bundle on GitHub Pages (Pages workflow runs on pushes to `main` or `deploy/github-pages`). Adapter-static runs with the repo base path `/BurBuriuok`, and a runtime `env.js` (written during CI) injects Supabase credentials for the browser. Legacy `first_draft/` prototype remains on GitHub Pages for archival reference.
