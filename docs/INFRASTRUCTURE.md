...
- **Data Storage** – Hosted Supabase project (`zvlziltltbalebqpmuqs`) for curriculum content, dependencies, concept progress, and audit logging (`content_versions`, `content_version_changes`). Migrations `0001`–`0009` in `supabase/migrations/` initialise the schema, add curriculum views, graft concept linkage, patch dependency constraints, and now persist rollback snapshots (`0009_db002_rollback_bundle.sql` applied remotely on 2025-11-13). Seeds stay in sync via the content scripts and `supabase/seeds/*`, while GitHub Actions emits `frontend/static/env.js` with production anon credentials during deploy.
