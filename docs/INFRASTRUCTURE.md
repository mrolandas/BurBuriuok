...
- **Integrations** – Supabase REST only; device-key header authenticates learner progress writes. Runtime Supabase URL/anon key are sourced from `window.__BURKURSAS_CONFIG__` so Pages builds remain static. Admin-only media bucket is being scoped (`media-admin` with signed URLs); learner-facing uploads remain deferred.
