<script lang="ts">
	...
	let admins: AdminProfileSummary[] = [];
	...
	async function handleRoleChange(profileId: string, role: ProfileRole): Promise<void> {
		...
		try {
			...
			admins = admins.map((admin) => (admin.id === profileId ? updated : admin));
		} catch (error) {
		...
		{:else}
			<section class="users__grid">
				...
				<article class="users__card">
					...
					{#if !admins.length}
						<p>Šiuo metu nėra registruotų administratorių.</p>
					{:else}
								...
								{#each admins as admin}
									<tr>
										<td>
											...
											{#if admin.callsign}
												<span class="muted">({admin.callsign})</span>
											{/if}
										</td>
										<td>
											<select
												bind:value={admin.role}
												on:change={(event) => handleRoleChange(admin.id, event.currentTarget.value as ProfileRole)}
												disabled={refreshingRoles.has(admin.id)}
											>
												{#each roleOptions as option}
													<option value={option.value}>{option.label}</option>
												{/each}
