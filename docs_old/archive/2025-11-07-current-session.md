...
5. **Cleanup & docs**
   ...
   - [x] Refresh `docs/references/features/implemented/ADMIN_SETUP.md` and user guides to explain the new inline editing flow.
...
- [ ] Update `docs/DEVELOPMENT_SETUP.md` with final run commands as services come online.
- [ ] Maintain API contract updates in `docs/references/API_CONTRACTS.md` as endpoints evolve.
- [ ] Capture admin UX refinements in `docs/references/features/implemented/ADMIN_SETUP.md` as screens ship.
...
- 2025-11-05: Promoted quiz blueprint to `docs/references/features/ideas/QUIZ.md`, defined dedicated `learner_quiz_metrics` table linked to `auth.users`, and opened LX-006 issue [#9](https://github.com/mrolandas/BurBuriuok/issues/9).
...
- 2025-11-06: Quiz discovery update – decided to keep question formats modular so sessions can mix configurable percentages (multiple choice, open response, image hotspot) and captured the new hotspot-on-image question concept in `docs/references/features/ideas/QUIZ.md`.
...
- 2025-11-06: ADM-002 concept editor MVP landed – `/api/v1/admin/concepts` routes plus SvelteKit drawer shipped with shared validation; docs refreshed (`BACKEND.md`, `API_CONTRACTS.md`, `ADMIN_SETUP.md`, `FRONTEND.md`, `SCHEMA_DECISIONS.md`).
...
- 2025-11-06: Global AppShell control renamed to “Aktyvuoti Admin”, now backed by the persisted `adminMode` store so impersonation survives navigation and updates the `impersonate=admin` query parameter automatically.
- 2025-11-06: Ran full regression commands (`npm test`, `npm run backend:typecheck`, `npm run frontend:check`, `npm run content:markdown:validate`, `npm run content:seed:check`) and refreshed documentation across `docs/` to capture the inline editing and admin toggle rollout.
